...
function stopContainers() {
  infoln "Stopping containers"
  ${CONTAINER_CLI} stop $(${CONTAINER_CLI} ps -aq --filter label=service=hyperledger-fabric) 2>/dev/null || true
  ${CONTAINER_CLI} stop $(${CONTAINER_CLI} ps -aq --filter name='dev-peer*') 2>/dev/null || true
  ${CONTAINER_CLI} stop "$(${CONTAINER_CLI} ps -q --filter name=ccaas)" 2>/dev/null || true
}
